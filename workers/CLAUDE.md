...
Seven independently-deployed Workers, one folder each. Shared server primitives live in `@avlo/worker-shared`; D1 + DO-SQLite Drizzle schemas in `@avlo/db`; typed HTTP-RPC clients live in `@avlo/api-client` (browser/SW side).
...
| **users** | `workers/users/` | `avlo-users` | 8793 | `users.avlo.io` | `DB` (D1 `avlo-db`), `AUTH` (service), `RL_ROOMS`, cross-script `rooms` (DO), `ROOM_MIGRATE` queue producer, queue consumers `avlo-room-visits`/`avlo-room-meta`/`avlo-room-migrate` (+DLQs) |
| **py** | `workers/py/` | `avlo-py` | 8794 | `py.avlo.io` | `PY` (R2 `avlo-py`) — nothing else: no auth, no rate limit, no services |
...
> `workers/` also carries two NON-deployed workspace members: `@avlo/test-support` (shared vitest plumbing — aux-worker esbuild, MSW wiring, stubs, byte builders) and `@avlo/integration` (the cross-worker `createTestHarness` suite). Neither has a wrangler config or a deploy script; both are test-graph only. See `TESTING.md`.
...
A pure assets-only worker. A worker *script* can be re-added later (redirects, etc.) without disturbing sync. Not run in dev (Vite serves the SPA); its real Static-Assets binding is exercised only by prod or a manual `wrangler dev` (see the `preview` note below — `pnpm preview` serves `web/dist` via Vite preview, not this worker's Static-Assets layer). (`pnpm preview` builds `web/dist` via `build:preview` — `vite build --mode preview`, local origins — then serves it with Vite preview alongside the local Miniflare workers.)
...
| `test/` | Pool-workers suite (upgrade guard, connect, permissions, hibernation via `evictDurableObject`, persistence, rate limiter) against the REAL auth aux worker; `harness.ts` = WS upgrade + minimal y-protocols client. Cross-worker pipelines (queues→D1) live in `workers/integration`. See `TESTING.md`. |
...
Cross-origin SPA → sync — the SPA on `avlo.io` opens `wss://sync.avlo.io/sync/rooms/<id>` (host from `SYNC_HOST`, prefix from `SYNC_WS_PREFIX`, in `web/src/runtime/room-doc-manager.ts`). `avlo.io → sync.avlo.io` is **same-site** (so `SameSite=Lax` permits the `.avlo.io` cookie on the upgrade) but **cross-origin** (so a true cross-site attacker is blocked by Lax — plus the explicit Origin guard in `on-before-connect`).
...
| `test/` | Suite: upload guard matrix, GET/Range/edge-cache, avatars, `ingestAvatar` (stub auth aux + in-isolate MSW). See `TESTING.md`. |
...
| `src/unfurl.ts` | HTMLRewriter OG extraction (page stream drained through a 2 MB cap), image fetch + R2 store (shared `avlo-assets`), edge cache 7d via `syntheticCacheUrl('unfurl', …)`. Every egress fetch (page, og-image, favicon, direct-image) rides `fetchGuarded` — per-hop SSRF guard incl. redirects (H9). |
| `src/app-type.ts` | Public mock + exported `UnfurlResponseBody` (real handler imports the type to constrain its data builder). |
| `test/` | Suite: validation (incl. every SSRF class), per-hop redirect guard, extraction precedence, image storage, edge cache — all egress via in-isolate MSW. See `TESTING.md`. |
...
| `src/session.ts` | Opaque KV sessions: 256-bit base64url token in `avlo_session`; KV record at `sess:<sha256hex(token)>` (a KV dump yields no usable tokens), `expirationTtl` 400 d + app-side `exp`. `readSession` (shape-gates the token before any I/O; KV `cacheTtl: 60` ⇒ documented ≤60 s revocation lag), `putSession`/`slideSession`/`deleteSession` (all `retryTransient` — critical writes), `mintSessionToken`. |
...
| `test/` | Suite: /me identity matrix, OAuth login + callback trust pipeline (real RS256 tokens via `google-mock.ts`), logout/CSRF, `verifySession` precedence + KV-outage degradation. Recording stubs stand in for USERS/IMAGES. See `TESTING.md`. |
...
| `src/queue.ts` | `consume` — ALL THREE queues (`switch` on `batch.queue`, defensive `default`); `safeParse` → ack-drop poison. **visits/meta:** coalesce (visits by `visitedAt`, meta by `rev`), then ONE `db.batch` of chunked multi-row upserts (≤99 bound params/statement — D1 caps 100) + `batch.ackAll()`; meta rows go through `upsertRoomsFromMeta` (@avlo/db — the same statement the PATCH handlers direct-write with), LWW by `excluded.rev >`, `createdAt` first-write-wins, `ownerId` rev-LWW. **migrate** (`avlo-room-migrate`, OAuth adopt overflow/retry — users is BOTH producer and consumer): PER-MESSAGE `ack`/`retry` (independent rooms, never `ackAll`/`retryAll`) calling `migrateOwner` + the per-room visit-copy, leaning on `migrateOwner` idempotency; `forbidden` = terminal `ack` skip, transient → `retry` → DLQ after `max_retries`. |
| `src/rpc.ts` | `UsersRpc.linkAccount(currentUserId, googleSub, {email, name, avatarHash})` — called only by auth's OAuth callback (§9). ONE atomic upsert on a `first-primary` session: `INSERT … ON CONFLICT(google_sub) DO UPDATE` (email/name refresh; `avatar_hash = coalesce(excluded, existing)` so a failed ingest never clobbers) `RETURNING` — new sub **promotes** the device id, existing sub **adopts** the account's userId. `user_id` PK conflict (device id already linked to a different account) is deterministic → caught → retried once with a fresh ulid; a same-account repeat sign-in dual-conflicts on the SAME row and resolves through the conflict target (verified). `withRetry` covers transient D1 failures; returns the post-coalesce `avatarHash` + RYW bookmark. Also `UsersRpc.migrateOwnedRooms(from, to, priorityRoomId?)` — the OAuth adopt owner-migration orchestrator, **never throws**: enumerate live owned rooms (partial `idx_rooms_owner`) → order priority/private/public → bounded-concurrency `migrateOwner` over the sync slice (CAP 50, concurrency 10) → RYW `db.batch` (rooms re-own via `upsertRoomsFromMeta` + `visitCopyStmt`, bookmark out) → overflow + transient/RYW-failed rooms to `ROOM_MIGRATE` (sendBatch chunks of 100). `forbidden` = terminal skip. `implements UsersRpcSurface` pins the surface. |
...
| `test/` | Suite: rooms-list projection/redaction, PATCH→REAL cross-script `AvloDO` (aux sync worker) with RYW + rev-guard, all three queue consumers via `createMessageBatch`, `UsersRpc` promote/adopt/migration. Real D1 via `applyD1Migrations`. Real-broker queue delivery + the real DO-thrown `forbidden` in a queue context live in `workers/integration`. See `TESTING.md`. |

Globally auth-gated. D1 is the sole schema owner (`@avlo/db`). Dev: Vite `/api/users/*` proxy.

### `workers/py/` — Python runtime artifacts (anonymous immutable serving)
| File | Responsibility |
|---|---|
| `src/index.ts` | `createCors({methods:['GET'],…})` → `devRequestLogger` → `cspHeaders('asset-body')` app-wide → `GET /:hash/:file` + `GET /:hash/bundles/:name` (Zod: 16-hex hash, `/`-barred names — `pyArtifactParam`/`pyBundleParam` in worker-shared) → `cspError`. `servePyObject`: raw `Accept-Encoding` read → `.br` sibling key with `Content-Encoding: br` + `encodeBody: 'manual'`, null-fallback to identity; `application/wasm` MIME (instantiateStreaming), `immutable` + `ETag` + `Vary: Accept-Encoding`; cold 304 via R2 `onlyIf`. **Edge cache** (`caches.default`): the encoding class rides a synthetic key (H7 — `py/<br\|id>/<r2 key>`) so br↔identity variants can never collide (the hazard a bare-URL key would create); bodies stored WITHOUT `Content-Encoding` (opaque bytes + `X-Avlo-Enc` marker — sidesteps the Cache API's compressed-payload transcoding entirely; CE + `encodeBody` re-attached on hit); warm 304 = manual `If-None-Match` vs cached ETag; 200s only, `tee()` + `waitUntil(cache.put)`, fail-open. (workerd normalizes the AE a worker sees — explicit `identity` is honored, absent AE reads as br-capable — the variant key stays correct under all of it.) |

Anonymous GET only (public build artifacts; every upper layer caches). Keys are `<buildHash>/…` from the committed `@avlo/py-loader` build-lock; R2 is seeded by py-build's `avlo-build publish` (`pnpm py:seed` local / `publish:r2` remote; restage ⇒ reseed). Like `sync`, **exempt from the App-Type pattern** — binary artifacts fetched by constructed URL, no typed JSON client. Dev: Vite `/api/py/*` proxy.

## App-Type Pattern (Option H)

Each worker that exposes a typed HTTP-RPC client to `@avlo/api-client` (auth, users, images, unfurl — `main` is assets-only, `sync` is WSS-only, and `py` is binary-artifact-only, so those are exempt) has a **public mock** in `src/app-type.ts` separate from the real handler in `src/index.ts`. The mock encodes the wire shape (paths, methods, validators, response types) ambient-free so client-side typecheck can traverse it without pulling worker ambient types (`Env`, `R2Bucket`, `HTMLRewriter`, `caches.default`).
...
| H9 | SSRF guard runs in a Zod `.refine` for the user-supplied URL, AND per egress hop for everything else — og:image/favicon URLs are attacker-authored page content, and redirects can land anywhere. No port allowlist (deliberate): workers egress can't reach arbitrary-port internal services the way a datacenter host can, and the host gate is the load-bearing check — revisit only if unfurl ever runs beside port-sensitive infrastructure. | `unfurlQuery` chain + `fetchGuarded` (worker-shared) on every unfurl/avatar egress fetch |
...
**`preview` + the `avlo` site worker.** The dev orchestrator does NOT run the `avlo` site worker (Vite serves the SPA in dev), so main's real Static-Assets binding is exercised only by prod or a manual `wrangler dev -c workers/main/wrangler.jsonc`. `pnpm preview` serves the built SPA via Vite preview; its script runs `build:preview` (`vite build --mode preview` → local origins) first and launches the local Miniflare workers alongside, so `web/dist` is produced automatically and the SPA reaches the workers through the Vite `/api/*` + `/sync` proxy (the whole point of the preview-mode target — a production-mode `vite build` would bake the `*.avlo.io` subdomains and bypass the proxy).
...
| Assert 403/400 on a rejected WS upgrade in tests | hono-party swallows the guard's response → the browser (and `SELF.fetch`) sees **404**. Assert 404; denial-with-reason is the DO's 4401/4403 close code |
