...
Anonymous GET only (public build artifacts; every upper layer caches). Keys are `<buildHash>/…` from the committed `@avlo/py-loader` build-lock; R2 is seeded by py-build's `publish.mjs` (`pnpm py:seed` local / `publish:r2` remote; restage ⇒ reseed). Like `sync`, **exempt from the App-Type pattern** — binary artifacts fetched by constructed URL, no typed JSON client. Dev: Vite `/api/py/*` proxy.
