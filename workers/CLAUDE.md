...
| `src/env.ts` | `UsersEnv = { Bindings: RefineBindings<Env, { AUTH: AuthRpcSurface }>; Variables: { userId: UserId } }` — `AUTH` retyped to its RPC surface for `requireAuth` (the cross-script `rooms` DO stays untyped, reached via `roomDoStub`); threaded through every handler (Hono `Context` is invariant in `Variables`). |
