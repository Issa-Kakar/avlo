...
| **ai** | `workers/ai/` | `avlo-ai` | 8794 | `ai.avlo.io` | `AvloAiAgent` (DO/SQLite, per user:room, Agents SDK/AIChatAgent) + `AI_QUOTA` (DO/SQLite `AiQuota`, per user), `AUTH` (service), `AI` (Workers AI, `remote: true`), `IMAGES` (R2 `avlo-assets`, shared), `RL_AI` (ns **1005**), secrets `GOOGLE_AI_STUDIO_API_KEY` (+ optional `AI_GATEWAY_*`, `AI_IMAGES_ENABLED`). WSS+HTTP on `/agents/*`; account-gated (Google only) at the edge. See `workers/ai/CLAUDE.md` + `docs/ai/` |
