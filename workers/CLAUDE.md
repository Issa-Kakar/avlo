...
| **users** | `workers/users/` | `avlo-users` | 8793 | `users.avlo.io` | `DB` (D1 `avlo-db`), `AUTH` (service), `RL_ROOMS`, cross-script `rooms` (DO), queue consumers `avlo-room-visits`/`avlo-room-meta`/`avlo-room-migrate` (+DLQs) + producer `ROOM_MIGRATE` (self-queue — the §9 adopt sweep) |
...
| `src/queue.ts` | `consume` — THREE queues (`switch(batch.queue)`); `safeParse` → ack-drop poison; coalesce (visits by recency `visitedAt`, meta by `rev`, migrate by `prevOwner\|nextOwner`), then ONE `db.batch` of chunked multi-row upserts (≤96 bound params/statement — D1 caps 100). Meta rows go through `upsertRoomsFromMeta` (@avlo/db — the same statement the PATCH handlers + the migration fan-out direct-write with). LWW guarded by `excluded.rev >` (visits by `last_visited_at >`); createdAt first-write-wins, owner rev-LWW. The `avlo-room-migrate` branch re-enumerates live owned rooms (`first-primary` session) + re-fans-out the idempotent `migrateOwner` in bounded concurrency, re-enqueuing a sweep while more than one wave remains. |
| `src/rpc.ts` | `UsersRpc.linkAccount(currentUserId, googleSub, {email, name, avatarHash})` — called only by auth's OAuth callback (§9). ONE atomic upsert on a `first-primary` session: `INSERT … ON CONFLICT(google_sub) DO UPDATE` (email/name refresh; `avatar_hash = coalesce(excluded, existing)` so a failed ingest never clobbers) `RETURNING` — new sub **promotes** the device id, existing sub **adopts** the account's userId. `user_id` PK conflict (device id already linked to a different account) is deterministic → caught → retried once with a fresh ulid; a same-account repeat sign-in dual-conflicts on the SAME row and resolves through the conflict target (verified). `withRetry` covers transient D1 failures; returns the post-coalesce `avatarHash` + RYW bookmark. `migrateOwnership(prevOwner, nextOwner)` — the §9 adopt fan-out (called by the callback AFTER session mint so it can't fail-close sign-in): no-op for PROMOTE, else enumerate owned-live rooms (partial index), migrate ≤`MIGRATE_CAP` synchronously (bounded concurrency, per-room `withRetry`), direct-write to D1, hand overflow/failures to the `avlo-room-migrate` sweep, and return the RYW bookmark. `implements UsersRpcSurface` pins the surface. |
