...
// Update existing buildSnapshot() to ensure proper freezing
...
private async loadCachedSnapshot(): Promise<void> {
  ...
  try {
    ...
    const currentSvKey = btoa(String.fromCharCode(...stateVector));
...
export interface WriteQueueConfig {
  maxPending: number;
  isMobile: boolean;
  getCurrentSize: () => number; // Get current doc size in bytes
}
...
export class CommandBus {
  ...
  private async executeCommand(cmd: Command): Promise<void> {
    ...
    // CRITICAL: Each command in exactly one transaction
...
describe('Scene Capture Consistency', () => {
  ...
  it('should handle rapid scene changes (property test)', () => {
    ...
    fc.assert(fc.property(
      fc.array(fc.oneof(
        fc.constant('pointerDown'),
        fc.constant('clearBoard'),
        fc.constant('pointerUp')
      )),
      (events) => {
