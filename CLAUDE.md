...
`packages/py-build` is the Python **toolchain** (the uv-run `avlo-build` CLI + `docker/fork.Dockerfile` for the fork build; the recipes lane is still a manual docker script; pipeline DAG in turbo — `pnpm py:board`) — never imported, only run. It is the sole member of the repo-root **uv workspace** (root `pyproject.toml` + committed root `uv.lock`).
