...
import { AVAILABLE_PACKAGES, PACKAGE_TO_SET, SET_BUNDLES, STDLIB_MODULES } from './py-stdlib-modules.gen';
...
/** `import a, b.c` (capture 1) or `from x.y import …` (capture 2). */
const IMPORT_STMT_RE = /^\s*(?:import\s+(.+)|from\s+([A-Za-z_][\w.]*)\s+import\b)/;
/** Leading dotted name of one `import` list item (`a.b as x` → `a.b`). */
const IMPORT_ITEM_RE = /^\s*([A-Za-z_][\w.]*)/;
const LINE_CONTINUATION_RE = /\\\r?\n/g;

/** Root package of a dotted module path — `a.b.c` → `a`. */
function rootOf(dotted: string): string {
  const dot = dotted.indexOf('.');
  return dot < 0 ? dotted : dotted.slice(0, dot);
}
...
export function scanPythonImports(source: string): string[] {
  ...
  const joined = source.replace(LINE_CONTINUATION_RE, ' ');
  ...
  for (const rawLine of joined.split('\n')) {
    ...
    // opener carries into the next lines. Unquoted runs append as slices.
    let out = '';
    let runStart = 0;
    for (let i = 0; i < line.length; i++) {
      const c = line[i];
      if (c === '#') {
        out += line.slice(runStart, i);
        runStart = line.length;
        break;
      }
      if (c !== '"' && c !== "'") continue;
      out += line.slice(runStart, i);
    ...
    for (const stmt of out.split(';')) {
      const m = IMPORT_STMT_RE.exec(stmt);
      if (!m) continue;
      if (m[2] !== undefined) {
        names.add(rootOf(m[2]));
      } else {
        // `import a.b as x, c` — split on commas, take each root.
        for (const part of m[1].split(',')) {
          const name = IMPORT_ITEM_RE.exec(part)?.[1];
          if (name !== undefined) names.add(rootOf(name));
        }
      }
    }
  }
...
/**
 * Resolve scanned imports to a bundle set key or a refusal (against the
 * generated AVAILABLE_PACKAGES — what the staged artifacts actually provide).
 ...
 */
export function resolveImports(modules: string[]): PyImportResolution {
  const missing: string[] = [];
  const neededBundles = new Set<string>();
  for (const m of modules) {
    if (STDLIB_MODULES.has(m)) continue;
    const set = PACKAGE_TO_SET[m];
    if (set === undefined || !AVAILABLE_PACKAGES.has(m)) {
      missing.push(m);
      continue;
    }
  ...
  if (neededBundles.size > 0) {
    const needed = [...neededBundles];
    const covering = SET_KEYS_BY_SIZE.find((k) => needed.every((b) => SET_BUNDLES[k].includes(b)));
    ...
    if (covering === undefined) throw new Error(`resolveImports: no set covers [${needed.join(', ')}]`);
    setKey = covering;
  }
...
export function unavailableMessage(missing: string[]): string {
  ...
  const marquee = ['numpy', 'pandas', 'matplotlib', 'seaborn'].filter((p) => AVAILABLE_PACKAGES.has(p));
