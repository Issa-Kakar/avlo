...
| `py-snapshot.ts` | Snapshot container parse (fork header + avlo meta v1) + `PackedTree`/`packTree` (site-packages walked AFTER imports, one blob = one transferable) + the at-rest OPFS wrapper codec: `py/<buildHash>/<setKey>.snap` = meta JSON + container head verbatim + tree blob + sparse heap (non-zero 64 KiB pages) + **sha-256 trailer**; read verifies magic→buildHash (the lock binding — buildHash commits to every artifact/bundle sha)→setKey→segment arithmetic→sha, ANY failure = delete→null. Store is supervisor-only (`navigator.storage` inside functions; the executor realm scrubs `navigator`): `read/write/deleteSetSnapshot`, `createSyncAccessHandle` writes, LRU 4 files/1 GB, stale-buildHash GC on first touch, quota-swallowing (persistence is best-effort by contract). Dependency-free — no build-lock import |
...
| `py-loader.ts` | Fork boot wrapper — `_loadSnapshot` (container restore), `_preRestoreHook` for stacked images (fires INSIDE restoreSnapshot pre-heap-overwrite: rebuild MEMFS tree bytes+mtimes → `loadDynlibReplay` per recorded loadOrder → `restoreDsoHandles` → `dsoReplayDone`; drift throws "snapshot DSO table drift"), `_makeSnapshot` on generation boots (fork gates capture on it) |
