...
- **Same-origin realm stripped of ambient authority — the authoritative
  ...
  finder level (proven by the py-integration suite even with the guard
  stripped);
...
Five independent layers; the py-integration vitest project is the one that
matters most, because it exercises **shipped modules verbatim** (`py-harden`,
`py-harness`, `py-mount`, `py-protocol`, `py-snapshot`, `py-loader` +
`@avlo/py-loader`) against real fork boots rather than a re-implementation.
Both test suites run under the one turbo task name: root `pnpm test:py`.

| Layer | What | Where |
|---|---|---|
| vitest (unit) | pure AVS2 codec — xxh32 vectors, header round-trip + every rejection path, chunked read/abandon | `py-snapshot.test.ts` |
| vitest py-integration | the old harness board as five files — `harden` (scrub/freeze/0008/protocol/trampoline census) · `harvest` (seaborn + figure-harvest protocol) · `snapshot` (capture→AVS2→restore ± negatives) · `mount-parity` (walker ≡ tarfile zero-diff) · `lock` (staged tree vs committed lock) — against real fork boots over the staged tree | `web/tests/py-integration/` (`pnpm --filter @avlo/web test:py`) |
| pytest corpus lane | python-behavior corpus (self-asserting samples per group set, pure-python mounts, font + pillow pixel gates) via pytest-pyodide's node runtime | `packages/py-build/tests/corpus/` (`uv run pytest`) |
| e2e | real browser + real OPFS; parses the `py:trace` line schema | `e2e/py-snapshot.spec.ts` |
| codegen drift gate | `py-stdlib-modules.gen.ts` + `build-lock.json` byte-identity | `pnpm --filter @avlo/py-build py:stage-check` |
