...
> **P2 LANDED (Session 15 — owned dense snapshots; NOTES.md in py-build is
> authoritative):** fork is Pyodide **314.0.2** / CPython 3.14 / emsdk 5.0.3 /
> MAIN_MODULE=2, DSOs grouped 67→4 (P1.5: one `.avlo/<bundle>.so` per bundle,
> `_AvloGroupFinder` → LDSO registry hits). Snapshots are OWNED end-to-end
> (`SNAPSHOTS_ENABLED = true`): every set captures client-side at the
> pre-harden slot on its first cold boot (bake → fork-API meta → dense
> `HEAP8.slice` → supervisor assembles AVS2 → chunked OPFS write, fused
> xxh32, off the boot path) and restores through the fork's
> `_avloRestore.preBlit` seam (buildId assert → `growMemory` → DSO replay at
> recorded bases with hard table asserts → chunked OPFS→HEAPU8 blit →
> `finalizeBootstrap`); restore boots extract tars WITHOUT dlopen. Failures
> self-heal: probe/pre-blit → `exec-snap-invalid` → delete + cold in the SAME
> boot; post-blit pre-ready (`restored:true`) → poison + ONE cold respawn;
> restored gen's first-run hard failure → poison before the eager respawn.
> `idleTeardownMs` = 15 s. The old `_loadSnapshot`/baseline/`PackedTree`/AVS1
> prose below is DEAD — trust NOTES.md + the code over any snapshot wording
> below; full rewrite lands P5.
