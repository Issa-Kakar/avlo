...
| `py-supervisor.ts` | Worker. Executor lifecycle, wall clocks (30 s soft + 5 s hard grace; 2 s cancel grace), idle teardown (2 min), eager respawn, result synthesis. M2: fetches bundle tars per the dev manifest, sha256-verifies EVERY tar (stale-mix refusal), caches verified bytes in memory across generations (boot gets transferred COPIES), posts `downloading` progress, respawns when a run needs bundles the current generation lacks (supersets satisfy subsets) |
| `py-executor.ts` | Nested worker. Pyodide instance, bundle mounts (tar → MEMFS extract → `loadDynlib` per meta loadOrder, deps-first across bundles → `ensure_tzpath`), raw-write stdout/stderr hooks (flush ≥100 ms/≥8 KB inside the write callback — no timers run mid-Python), 4096-char output cap |
...
- **The executor never fetches.** Artifact fetches happen inside
  `bootPyodide`; bundle tars are fetched + sha256-verified by the SUPERVISOR
  against the staged manifest and arrive as transferred bytes — a drifted
  artifact mix is refused, never mounted.
