// GENERATED by packages/py-build/scripts/stage.mjs from dist/raw/pyodide.d.ts — DO NOT EDIT.
...
interface API {
	...
	dsoReplayDone: () => void;
	loadDynlibReplay: (path: string, bytes: Uint8Array | WebAssembly.Module) => void;
	restoreDsoHandles: (dsoHandles: Record<string, {
		handles: number[];
	}>) => void;
	recordDsoHandles: () => Record<string, {
		handles: number[];
	}>;
...
declare class PyodideAPI_ {
	...
	/**
	 * AVLO (0009): the live Emscripten Module and the internal API object —
	 * the fork-private surfaces the app's boot/capture/restore path types
	 * against. Assigned (untyped, upstream) in makePublicAPI below; declared
	 * here so the emitted pyodide.d.ts carries them. `declare` = type-only,
	 * zero runtime emission.
	 * @hidden
	 */
	static _module: PyodideModule;
	/** @hidden AVLO (0009): see _module. */
	static _api: PyodideModule["API"];
	/**
	 *
...
interface PyodideConfig {
	...
	/**
	 * AVLO owned restore (P2 — replaces upstream's _loadSnapshot/_makeSnapshot;
	 * capture is host-driven via API.serializeHiwireState + the dynload replay
	 * surface, so no loader config is needed for it). When present the runtime
	 * boots with noInitialRun (runtime init + preRun + main-module ctors still
	 * run; main() — CPython init — is skipped) and `preBlit(Module, API)` runs
	 * in bootstrapPyodide BEFORE finalizeBootstrap: the host validates its
	 * container, pre-grows memory (Module.growMemory — exported by the AVLO
	 * link flags; the upstream INITIAL_MEMORY resize was a no-op against this
	 * glue because memory is wasm-exported), replays recorded DSOs at their
	 * captured bases, blits the heap image, and returns the SnapshotConfig
	 * (hiwire state) that finalizeBootstrap resumes from.
	 * @ignore
	 */
	_avloRestore?: {
		preBlit: (module: PyodideModule, api: PyodideModule["API"]) => SnapshotConfig | undefined | Promise<SnapshotConfig | undefined>;
	};
	/**
	 * @experimental
