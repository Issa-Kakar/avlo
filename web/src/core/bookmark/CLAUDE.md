...
The drop + contact gaussian layers are computed analytically by `bakeRoundRectShadow` (`core/geometry/shadow-bake.ts`, erf-based SDF — no `ctx.shadowBlur`, no `destination-out`), so the cache is bit-identical across engines and holds only the soft halo (no opaque body). This replaced an opaque-fill-then-punch bake whose `destination-out` left an engine-dependent `c·(1−c)` residual — invisible on Skia Graphite but a visible corner blob + (via the 3-slice stretch) vertical side line on raster-Skia + Firefox. Body geometry uses `BOOKMARK_CORNER_RADIUS`, matching the destination body fill 1:1, and the analytic shadow is exactly translation-invariant along the straight side edges the 3-slice stretches. Seam invariant: each cap's innermost source row and the middle's sampled rows are all fully-invariant body rows (pixel-identical), so cap→middle→cap transitions are exact regardless of `midDestH`.
