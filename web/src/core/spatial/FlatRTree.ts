 ...
 * retains no high-water memory from the previous room. Load-engine scratch is
 * BUILD-TRANSIENT (allocated per load, released before load returns — nothing
 * survives at instance or module scope; see the engine section). `dispose()`
 * releases every last buffer for good — a disposed instance is dead. The
 * traversal stack is a FIXED 1024 words with no growth logic anywhere (SQLite
 * R-tree style): a DFS holds ≤ 15 pending siblings per level of the current
 * path and a covered-subtree dump nests one more walk of the same shape, so
 * the need is < 2·height·15 + 2. Height is structurally bounded: splits
 * create nodes with ≥ 7 of 16 entries, so filling one back to overflow takes
 * ≥ 10 child splits — each root level costs ~10× the inserts of the level
 * below (OMT loads are shallower still: ⌈log₁₆ 2³⁰⌉ = 8). Height 15 ≈ 10¹⁵
 * inserts; 2·15·15 ≪ 1024.
 *
 * ── Split heuristic ─────────────────────────────────────────────────────────
 ...
 * elongated-item boards they matter: a slice by minX pins a connector to the
 ...
 * this data it herds inserts into big sloppy leaves (their Δoverlap ≈ 0).
 * Don't re-add without new numbers.
 *
 * ── Bulk-load engine ────────────────────────────────────────────────────────
 ...
 * the first call in a fresh context (hydrate), i.e. Ignition/Sparkplug/
 * early-Maglev, where cost ≈ element-touches × (dispatch + boxing), not
 * machine ops:
 ...
 * (10k), fresh-tree query quality ~10% better via the center keys.
 *
...
export class FlatRTree {
  ...
  private _buildFrom(count: number, bIds: Uint32Array, bBoxes: Float64Array): void {
    ...
    const subAnn = this._buildNode(bIds, bBoxes, 0, count - 1, count <= 16 ? 0 : height, rootFanout, rootShift);
    this._lk = EMPTY_I32;
    this._li = EMPTY_I32;
    this._tk = EMPTY_I32;
    this._ti = EMPTY_I32;
   ...
   * index over EIGHT candidates — k ∈ [7,10] × {min order, max order of the
   * winning axis} — by minimum overlap, tie minimum combined area. The
...
function seedKeys(bBoxes: Float64Array, count: number, kx: Int32Array, ky: Int32Array, lk: Int32Array, li: Int32Array): number {
...
function refillKeys(keySrc: Int32Array, lk: Int32Array, li: Int32Array, lo: number, hiEx: number): number {
  let mn = 2147483647;
  let mx = -2147483648;
...
function histPass(lk: Int32Array, h: Int32Array, lo: number, hiEx: number, shift: number, bx: number): void {
