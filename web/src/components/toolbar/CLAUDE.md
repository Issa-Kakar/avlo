...
| `inspectors/InspectorButton.tsx/css` | Memoized square icon button with `is-active` state. 32×32, 7px radius, 20×20 icon — single size across all three inspectors so each pill's cross-axis is 48px (32 + 8×2 padding), matching the dock pill. Used by pen/highlighter toggles, weight buttons, connector variants, shape variants. Optional `onPointerDown` prop for drag-place starts; `touch-action: none` so touch drags aren't stolen by scroll gestures. |
...
| `inspectors/StickyNotePanel.tsx/css` | Sticky-note color popout — **not** a `.inspector` pill; its own wider near-black panel of 12 `NOTE_COLOR_PALETTE` swatches drawn as miniature sticky notes. Mounted by `Toolbar` while `stickyPanelOpen`. Swatch pointerdown → `beginNotePlace(e, fill)` (`runtime/input/toolbar-place.ts`) — sets the fill and starts a capture-retargeted drag-place (drop creates the note + opens its editor; sub-threshold release = pick-only). The `onClick` (`setNoteFillColor` + `closeStickyPanel`) survives as the guard-refused fallback. `touch-action: none` on swatches. See "Sticky-Note Color Panel" below. |
...
`stores/history-store.ts` (its `bindUndoManagerToHistoryStore` binder pushes
Y.UndoManager stack state into the store, and the buttons use native
`disabled` to grey out when the action is unavailable).
...
**Drag-to-create (shipped).** Pointerdown on a swatch (or a shape-inspector
variant button) routes through `runtime/input/toolbar-place.ts`: apply the
selection, `tool.beginPlace(...)`, capture the pointer to the canvas, set the
`grabbing` cursor override. Capture retargets all further pointer events to
the canvas, so the existing `InputManager → CanvasRuntime → tool.move/end`
dispatch runs the gesture and every `isActive()`-based keyboard guard holds.
A WYSIWYG preview (180wu shape / `NotePreview` empty sticky) follows the
cursor from the first move; release ≥5 screen px from the press commits at
the drop point (notes also mount their editor), below it degrades to the
plain click semantics above. Capture also retargets the `click` event away
from the button, so the `onClick` handlers only fire on guard-refused presses.
