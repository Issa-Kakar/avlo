...
//   node scripts/analyze-dsos.mjs --check   # gate mode (dsos:check). Grouped
//                                           # world (tars ship .avlo/<b>.so):
//                                           # exact PyInit census equality vs
//                                           # config/dso-groups/groups.json,
//                                           # finder-derivable init names
//                                           # (PyInit_<last dotted component>),
//                                           # dylink.0 present + NEEDED empty,
//                                           # closed world vs main ∪ self ∪
//                                           # {exit}, loadOrder ≤1 under .avlo/,
//                                           # mixed-world hard fail. Always:
//                                           # PyInit shortname uniqueness per
//                                           # bundle + no PyInit-less DSO.
//                                           # (groups.json census updates come
//                                           # from the recipes-loop harvest
//                                           # manifests — the old --mint-groups
//                                           # pre-grouping lane is gone.)
//
...
// (mem/table sizes, NEEDED — shipped 314 DSOs carry MEM_INFO only), toolchain
// fingerprint from embedded strings. Derived: import-provider table (self
// counts FIRST — ODR/PIC modules export-and-reimport their own symbols;
// classifying self last invents phantom cross-DSO deps) + the lazy-stub audit.
...
const providers = { self: 0, main: 0, 'other-bundle': 0, 'js-glue': 0 };
...
for (const b of bundles) {
  ...
  for (const d of members) {
    ...
    if (!d.pyinit.length) failures.push(`DSO with no PyInit_* export: ${b}/${d.soName}`);
  }
