...
for (const v of vs) {
  ...
  P(
    `| ${LABEL[v] ?? v} | ${(s.wasmBytes / 1e6).toFixed(2)} MB | ${(s.brotliBytes / 1e6).toFixed(2)} MB | ${sp(pct(s.brotliBytes, bBr))} | ${s.bodyCount} | **${s.evalFrameTotal.toLocaleString()} B** | ${s.tailCallFuncs || '—'} | ${s.tailCallMedianBytes || '—'} B | ${s.dispatcherBytes || '—'} B |`,
  );
}
P();
...
for (const v of vs) {
  P(
    `| ${LABEL[v] ?? v} | ${R.validate[v] ? R.validate[v].median.toFixed(1) : '—'} | ${R.startupBare[v]?.median ?? '—'} | ${R.startupBare[v] ? sp(pct(R.startupBare[v].median, bSt)) : '—'} |`,
  );
}
P();
...
for (const v of vs) {
  const rs = [];
  const cells = B.map((b) => {
    const a = R.cold[BASE]?.[b],
      c = R.cold[v]?.[b];
    if (!a || !c) return '—';
    const r = a.median / c.median;
    rs.push(r);
    return r.toFixed(2);
  });
...
for (const [eng, tier, title] of [
  ...
  for (const v of vs) {
    ...
    const cells = B.map((b) => {
      ...
      const x = base[b].median / r[b].median;
      rs.push(x);
      return x.toFixed(2);
    });
    P(`| ${LABEL[v] ?? v} | ${cells.join(' | ')} | **${geo(rs).toFixed(3)}** |`);
