...
for (const v of variants) {
  ...
  console.error(
    `  ${v.padEnd(8)} wasm=${(s.wasmBytes / 1e6).toFixed(2)} br=${(s.brotliBytes / 1e6).toFixed(2)} eval=${s.evalFrameTotal} tc=${s.tailCallFuncs} disp=${s.dispatcherBytes}`,
  );
}
...
for (const v of variants) {
  try {
    R.validate[v] = grabJson(run(ENGINES.node26, [`${TC}/compile-bench.mjs`, join(BUILDS, v, 'python.wasm'), '5']), 'COMPILE_JSON:');
    console.error(`  ${v.padEnd(8)} ${R.validate[v].median.toFixed(1)}ms`);
  } catch (_e) {
    console.error(`  ${v} FAIL`);
  }
}
  ...
  for (const [tier, flags] of Object.entries({ default: [], liftoff: ['--liftoff-only'] })) {
    ...
    for (const v of variants) {
      ...
      try {
        ...
        console.error(`  ${key.padEnd(28)} FAIL`);
      }
    }
  }
}
