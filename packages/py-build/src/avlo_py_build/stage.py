...
def run(args) -> int:
    ...
    for marker in (b"snapshot DSO table drift", b"loadDynlib"):
        if marker not in glue:
            sys.exit(f'built glue is missing "{marker.decode()}" — rebuild with the full patch queue (avlo-build fork)')
    ...
    builtins = json.loads((RAW_DIR / "builtin-modules.json").read_text())  # emitted by the fork build
...
// dist/raw/builtin-modules.json (avlo-build fork), build.config.json,
...
"""
    ...
    if not dts_raw_path.exists():
        sys.exit("missing dist/raw/pyodide.d.ts — rebuild (avlo-build fork) first")
