 ...
 */
import { ROOM_CONFIG, STROKE_CONFIG, PERFORMANCE_CONFIG, isRoomReadOnly, isRoomSizeWarning, getRoomSizePercentage, } from '@avlo/shared';
...
export function validateStrokeBeforeCommit(points, tool) {
...
export class FrameRateManager {
    ...
    getMicroBatchWindow(isUnderPressure) {
        if (isUnderPressure) {
...
export function canJoinRoom(currentClients) {
    return currentClients < ROOM_CONFIG.MAX_CLIENTS_PER_ROOM;
}
export function getRoomCapacityMessage(currentClients) {
...
export function useRoomSizeMonitor(sizeBytes) {
