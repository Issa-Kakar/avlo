...
export function computeSelectionComposition(ids: string[]) {
  ...
  let strokes = 0, shapes = 0, text = 0, connectors = 0;
  ...
  for (const id of ids) {
    ...
    switch (handle.kind) {
      case 'stroke': strokes++; break;
      case 'shape': shapes++; break;
      case 'text': text++; break;
      case 'connector': connectors++; break;
    }
  }

  const kindCounts: KindCounts = {
    strokes, shapes, text, connectors, total: selectedIdSet.size,
  };

  const nonZero = (strokes > 0 ? 1 : 0) + (shapes > 0 ? 1 : 0)
    + (text > 0 ? 1 : 0) + (connectors > 0 ? 1 : 0);
  ...
  const mode = selectedIdSet.size === 1 && selectionKind === 'connectorsOnly'
    ? 'connector' as const
    : selectedIdSet.size > 0 ? 'standard' as const : 'none' as const;
