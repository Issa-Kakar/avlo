...
export class TextTool implements PointerTool {
  ...
  private mountEditor(objectId: string, isNew: boolean): void {
    ...
    if (isLabel) {
      ...
      const padding = getNotePadding(fontSize);
      const contentWidth = getNoteContentWidth(noteWidth as number, fontSize);
  ...
  private commitAndClose(): void {
    ...
    if (this.editor.isEmpty) {
      ...
      } else if (!handle || !isNote(handle.y)) {
