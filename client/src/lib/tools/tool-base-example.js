...
export class DrawingToolExample {
    roomDocManager;
    sceneCapture = new SceneCapture();
    currentStroke = null;
    constructor(roomDocManager) {
        this.roomDocManager = roomDocManager;
    }
    /**
    ...
    handlePointerDown(event) {
        ...
        const capturedScene = this.sceneCapture.capture(this.roomDocManager.currentSnapshot);
    ...
    handlePointerUp(_event) {
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            console.log(`[DrawingTool] Committing stroke: capturedScene=${capturedScene}, currentScene=${currentScene}`);
            if (capturedScene !== currentScene) {
                console.warn(`[DrawingTool] Scene changed during gesture! This is EXPECTED and HANDLED CORRECTLY.`);
            }
        }
...
export class MultiTouchDrawingExample {
    roomDocManager;
    sceneCaptureManager = new SceneCaptureManager();
    ...
    handleTouchStart(event) {
        ...
        Array.from(event.changedTouches).forEach(touch => {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log(`[MultiTouch] Touch ${touch.identifier} started at scene=${capturedScene}`);
            }
        });
    }
    ...
    handleTouchEnd(event) {
        ...
        Array.from(event.changedTouches).forEach(touch => {
            ...
            if (stroke) {
                ...
                if (process.env.NODE_ENV === 'development') {
                    console.log(`[MultiTouch] Touch ${touch.identifier} committed with scene=${capturedScene}`);
                }
                this.roomDocManager.write(command);
...
export class ChunkedDrawingExample {
    ...
    startChunkedOperation() {
        // Capture scene once at the start
        this.initialScene = this.sceneCapture.capture(this.roomDocManager.currentSnapshot);
        this.chunkBuffer = [];
    }
    addChunk(points) {
