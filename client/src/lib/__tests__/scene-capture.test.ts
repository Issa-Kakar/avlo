...
import { SceneIdx, Snapshot, PresenceView, RBushIndexView, ViewTransform, SnapshotMeta } from '@avlo/shared';
...
describe('SceneCapture - Distributed Systems Causal Consistency', () => {
  ...
  const createSnapshot = (scene: SceneIdx): Snapshot => ({
    svKey: 'test-key',
    scene,
    strokes: [],
    texts: [],
    ...
    spatialIndex: {} as RBushIndexView,
    view: {
      scale: 1,
      pan: { x: 0, y: 0 },
  ...
  describe('Core Functionality', () => {
    it('should capture scene at interaction start', () => {
      const snapshot = createSnapshot(5 as SceneIdx);
    ...
    it('should throw when getting required scene without capture', () => {
      expect(() => capture.getRequired()).toThrow(
        'Scene not captured - MUST call capture() at interaction start'
      );
    });
    
    it('should return captured scene with getRequired after capture', () => {
      const snapshot = createSnapshot(3 as SceneIdx);
    ...
    it('should reset capture state', () => {
      const snapshot = createSnapshot(2 as SceneIdx);
      capture.capture(snapshot);
    ...
    it('should validate capture age', () => {
      const snapshot = createSnapshot(1 as SceneIdx);
      capture.capture(snapshot);
  ...
  describe('Distributed Systems Edge Cases', () => {
    ...
    it('should detect and reject scenes from the future', () => {
      ...
      expect(() => capture.validateNotFuture(currentScene)).toThrow(
        'Scene from future detected! captured=5 > current=3'
      );
    });
    ...
    it('should warn about stale captures in development', () => {
      ...
      const snapshot = createSnapshot(0 as SceneIdx);
      capture.capture(snapshot);
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Using stale capture')
      );
  ...
  describe('SceneCaptureManager for Multi-Touch', () => {
    let manager: SceneCaptureManager;
  ...
  describe('assertSceneConsistency', () => {
    ...
    it('should warn for large scene deltas', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Large scene delta in DrawStrokeCommit: delta=11')
      );
    ...
    it('should pass for valid scene consistency', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('consistency check: captured=3, current=3, delta=0')
      );
  ...
  describe('Property-Based Testing', () => {
    it('should maintain causal consistency for any event sequence', () => {
      ...
      let capturedScene: number | null = null;
      let currentScene = 0;
      
      events.forEach(event => {
        if (event.type === 'pointerDown') {
          const snapshot = createSnapshot(currentScene as SceneIdx);
