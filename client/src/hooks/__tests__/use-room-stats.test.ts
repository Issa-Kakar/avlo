...
describe('useRoomStats', () => {
  ...
  it('should update when stats change', async () => {
    ...
    const subscribers = new Set<(s: RoomStats | null) => void>();
  ...
  it('should handle room at capacity', async () => {
    ...
    const atCapacityStats: RoomStats = {
  ...
  it('should handle stats updates during rapid re-renders', () => {
    ...
    expect(
      result.current === null ||
        (result.current?.bytes !== undefined && result.current?.cap !== undefined),
    ).toBe(true);
  });
});
