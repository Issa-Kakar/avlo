...
| `code-theme.ts` | CodeMirror theme extensions — lazy-loaded dark theme + syntax highlighting (`getCodeMirrorExtensions`). All `@codemirror/*` AND `@lezer/highlight` imports are DYNAMIC (inside the async function) so the main bundle has zero static dependency on the editor stack. Builds its `HighlightStyle.define` rule list inline, deriving colors from `THEME.palette`. No bold or italic — Sweet Dracula's emphasis is color-only. No dependency on code-system |
| `lezer-worker.ts` | Web Worker — per-object Lezer `Tree` + `TreeFragment` state, cached configured parsers, incremental parsing, `highlightTree(tree, STYLE_HIGHLIGHTER, …)` → flat `{spanData, spanLineStart}` via `+classes \| 0` + `writePackedTriples`, zero-copy transfer of both ArrayBuffers. Owns its own inline `WORKER_RULES` (tag → S) + custom `STYLE_HIGHLIGHTER` — duplicated with `code-theme.ts` on purpose so `@lezer/highlight` stays out of the main bundle |
...
### Lezer-tag → S Mapping (two sources of truth — deliberate)

The Lezer-tag → S mapping is duplicated in two places, on purpose:

- **`code-theme.ts`** (lazy, runs in main bundle's CodeMirror chunk) builds the inline rule list for `HighlightStyle.define([...])` — each row is `{ tag: Tag[], color: THEME.palette[S.X] }`. The list lives INSIDE `getCodeMirrorExtensions()` because `tags` is dynamically imported alongside `@codemirror/*`.
- **`lezer-worker.ts`** (Web Worker bundle) declares `WORKER_RULES: { tags: Tag[]; style: S }[]` at module scope and builds a tag→S `Map` + pre-stringified `_classCache`. `STYLE_HIGHLIGHTER`'s `style(tagList)` walks each tag's modifier `set` for the first match and returns the stringified S int directly; the `highlightTree` callback recovers via `+classes | 0`.

**Why two sources of truth?** Keeping `@lezer/highlight` (and its `Modifier` / `Tag` machinery) out of the main bundle. A shared file would force a static import of `@lezer/highlight` into whichever module references the rules, which bleeds into `main-*.js`. By duplicating, the theme module's only Lezer dependency is dynamic (chunk-split with `@codemirror/*`), and the worker module's Lezer dependency is in the worker chunk — neither shows up in the main bundle.

**Invariant:** the two rule tables MUST stay in sync. If a row changes, change it in both files. The shape is intentionally identical (same row order, same tag groupings) so a diff is trivial.

**Modifier-set walk behavior** (same on both sides):
- HighlightStyle.define uses `@lezer/highlight`'s `tagHighlighter(specs)` internally — walks each tag's `set` for the first matching rule.
- The worker's custom `STYLE_HIGHLIGHTER` walks `set` manually with the same semantics.
- Both rely on Modifier.id ordering inside the `set` (`definition` < `function`), which is a property of `@lezer/highlight`'s declaration order.
...
**Worker bundle hygiene.** `code-tokens.ts` imports `code-system.ts` only as `import type { CodeSpans }` — value imports are forbidden. A static value import would chain `code-tokens → code-system → @/renderer/RenderLoop → @/core/image/image-manager`, dragging the main-thread bundle into the worker; `image-manager.ts` has top-level `new Worker(...)` (would spawn 2 nested image workers per lezer worker on module load) and `window.addEventListener(...)` (throws in worker context), aborting the worker's module-load before `onmessage` is installed and silently dropping every parse request. The cap helpers (`ensureSpansDataCap` / `ensureSpansLineCap`) live in `code-tokens.ts` to keep this chain broken. `w.onerror` / `w.onmessageerror` handlers in `ensureWorkers` surface any future module-load regression instead of failing silently.

**Main bundle hygiene.** `code-theme.ts` has NO top-level `@lezer/highlight` import — `tags` is dynamically imported inside `getCodeMirrorExtensions()` so it ends up in the lazy CodeMirror chunk, not in `main-*.js`. The worker carries its own copy of the rule list (`WORKER_RULES` + `STYLE_HIGHLIGHTER`). If you reintroduce a shared file that imports `@lezer/highlight` and `code-theme.ts` statically imports it, `Modifier` / `TagName` / `Tag` will bleed back into the main bundle (~30–40 kB of dead weight on first paint). Verify with `grep -l "Modifier\|TagName" dist/assets/main-*.js` after each build.
