...
export interface KindCapability<K extends ObjectKind> {
  ...
  /** Occlusion paint classification, used by pickers. */
...
const STROKE_CAP: KindCapability<'stroke'> = {
  ...
  hitPoint: (h, p, r) => {
    ...
    const tolerance = r + getWidth(h.y) / 2;
    if (!strokeHitTest(p, points, tolerance)) return null;
    return { distance: 0, insideInterior: false, area: computePolylineArea(points), isFilled: true };
  },
  hitRect: (h, bbox) => {
    const points = getPoints(h.y);
 ...
 * point/rect/circle math against a derived FrameTuple. Only the frame getter
 * and `isFilled` rule differ per kind.
 */
function framedCap<K extends 'text' | 'note' | 'code' | 'image' | 'bookmark'>(
  resolveFrame: (h: HandleOf<K>) => FrameTuple | null,
  resolveIsFilled: (h: HandleOf<K>) => boolean,
  classify: (c: HitCandidate<K>) => Paint,
): KindCapability<K> {
  return {
    bindable: true,
    frame: resolveFrame,
...
const TEXT_CAP = framedCap<'text'>(
  (h) => getTextFrame(h.id),
  (h) => !!getFillColor(h.y),
  (c) => (c.isFilled && c.insideInterior ? 'fill' : 'ink'),
);

const NOTE_CAP = framedCap<'note'>(
  (h) => getTextFrame(h.id),
  () => true,
  () => 'ink',
);

const CODE_CAP = framedCap<'code'>(
  (h) => getCodeFrame(h.id),
  () => true,
  (c) => (c.insideInterior ? 'fill' : 'ink'),
);

const IMAGE_CAP = framedCap<'image'>(
  (h) => getFrame(h.y),
  () => true,
  () => 'ink',
);

const BOOKMARK_CAP = framedCap<'bookmark'>(
  (h) => getBookmarkFrame(h.id),
  () => true,
  () => 'ink',
);
