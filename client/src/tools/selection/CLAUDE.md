...
- Text: `frameToBbox(getTextFrame(id))` — `handle.bbox` carries italic-overhang pad; overlay needs the tight visual frame
- All others (including code): `handle.bbox` — `computeCodeBBox` writes `bbox = frameToBbox(frame)` so it's already tight
