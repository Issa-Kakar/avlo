...
var ShapeSpatialIndex = class {
  ...
  idBySlot = [];
  /** shape id -> slot. The mirror of the `Map<TLShapeId, SpatialElement>` that
   *  `RBushIndex` had to keep anyway, holding an int instead of an object. */
  slotOf = /* @__PURE__ */ new Map();
  /** Recycled slots, newest first. A typed stack, not a `number[]`. */
  freeSlots = new Uint32Array(64);
  freeCount = 0;
  nextSlot = 0;
  /** Staging for a full repopulate. Owned here so callers never see it. */
  stagedIds = new Uint32Array(0);
  stagedBoxes = new Float64Array(0);
  stagedCount = 0;
  getSize() {
   ...
   *  allocation — this is what `RBushIndex.getElement` existed for. */
   ...
   */
  search(minX, minY, maxX, maxY, narrow) {
    const out = /* @__PURE__ */ new Set();
    if (!isIndexableBox(minX, minY, maxX, maxY)) return out;
    const n = narrow ? this.tree.searchPrecise(minX, minY, maxX, maxY) : this.tree.search(minX, minY, maxX, maxY);
   ...
   */
  rebuildFrom(sizeHint, fill) {
    this.clear();
    this.growStaging(sizeHint);
    const add = (id, minX, minY, maxX, maxY) => {
